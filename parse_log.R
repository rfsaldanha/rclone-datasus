...
res <- tibble::tibble(
  ...
  dplyr::filter(!stringr::str_detect(log_info, "There was nothing to transfer")) |>
  ...
  dplyr::mutate(
    path = dplyr::case_when(
      stringr::str_detect(log_info, " INFO  : ") ~ stringr::str_sub(log_info, 29, 1000000)
    ),
  ...
  dplyr::mutate(
    action = dplyr::case_when(
      ...
      stringr::str_ends(log_info, stringr::fixed("Copied (replaced existing)")) ~ "Replaced",
